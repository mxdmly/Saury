...
QMutex qlock;
QDateTime upTime = QDateTime(QDate(2018, 1, 18), QTime(1, 0, 0, 0));
QDateTime thisTime;
QString cxBAS_str;
...
void Caviar::run()
{
    ...
    while (ifRun_b) {
        ...
        if(1|upTime.operator <=(thisTime))//if thisTime >= upTime
        {
...
//------------------------------------------------------------------------------------------
            ...
            log_str = "";
            log_str = QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss ddd");
...
void Caviar::getDataTABLEUPLOAD()
{
    ...
    while (cxBAS_qsq->next()) {
        ...
        if(crEtone_qsq->exec(crEtone_str))get_i++;
    }
    writeLog(crEtone_str);
...
void Caviar::getDataTABLEB()
{
    ...
    cxBAS_str.append("> ").append(descTOP_str).append(" ORDER BY dbo.TBABYCARD.FID ASC");
