...
void Caviar::getDataTABLEUPLOAD()
{
    ...
    if(all_i != get_i)log_str.append("Error all " + all_i).append(" get " + get_i);
}
...
void Caviar::getDataTABLEZ()
{
    ...
    QString crEtoneMin_str = openSqlFile("C:\\Saury\\qIntoZ_not.sql");
    ...
    all_i = cxBAS_qsq->numRowsAffected();
    while (cxBAS_qsq->next()) {
        if("1" == cxBAS_qsq->value(4).toString() || 1 == cxBAS_qsq->value(4).toString())
        {
        ...
        else //减少SQLSERVER冗余，不重复插入非主要诊断的信息
        {
            crEtone_str = crEtoneMin_str;
            crEtone_str.append("\'").append(cxBAS_qsq->value(0).toString()).append("\'");
            for (int i = 1; i < 9; ++i) {
                crEtone_str.append(",\'").append(cxBAS_qsq->value(i).toString()).append("\'");
            }
